...
cp /path/to/data/bugbot.db /backups/bugbot-$(date +%F).db

# Online backup (no full stop)
sqlite3 /path/to/data/bugbot.db ".backup '/backups/bugbot-$(date +%F).db'"
```
...
2. Replace `bugbot.db` with the backup file.
...
mkdir -p "$DRILL"
cp -a data/bugbot.db "$DRILL/bugbot.db"
...
cp -a "$DRILL/bugbot.db" "$RESTORE/data/"
...
2. Replace `data/bugbot.db` with the known-good file from `deployment-backups/drill-*/bugbot.db`.
